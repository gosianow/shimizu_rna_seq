...
# sum( rowSums(d.cpm.org > 0.1) > 10 )

# cpm cutoff hihg:
#d.org <- d.org[ rowSums(d.cpm.org>1) > 2, ]
# cpm cutoff relaxed:
d.org <- d.org[ rowSums(d.cpm.org > 0.1) > 10 , ]
d.cpm.org <- cpm(d.org, normalized.lib.sizes=TRUE)
...
PAM.clustering <- function(pam.x, new.samps, genes.full.description,  dist.method= "spearman", norm.method="norm", out.path="Clustering/PAM/", out.name="", prior.nr.cl=2:5, mc.cores=1, ylim=c(-3,4)){
...
# voom 
# pam.x <- count.data$d.voom.weight[selected.genes$var.genes.voom, ]
# PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "spearman", norm.method="norm", out.path="Clustering/PAM_voom/", out.name="voom_spearman_norm", prior.nr.cl=2:15, mc.cores=1)
# PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "spearman", norm.method="01", out.path="Clustering/PAM_voom/", out.name="voom_spearman_01", prior.nr.cl=2:15, mc.cores=1)



# DESeq vsd
pam.x <- count.data$vsd[selected.genes$var.genes.vs, ]


PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "spearman", norm.method="norm", out.path="Clustering/PAM_DESeq/spearman/", out.name="DESeq_spearman_norm", prior.nr.cl=2:25, mc.cores=1, ylim=c(-3,4))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "spearman", norm.method="01", out.path="Clustering/PAM_DESeq/spearman/", out.name="DESeq_spearman_01", prior.nr.cl=2:25, mc.cores=1, ylim=c(-0.2,1.2))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "spearman", norm.method="none", out.path="Clustering/PAM_DESeq/spearman/", out.name="DESeq_spearman_none", prior.nr.cl=2:25, mc.cores=1, ylim=NULL)


PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "pearson", norm.method="norm", out.path="Clustering/PAM_DESeq/pearson/", out.name="DESeq_pearson_norm", prior.nr.cl=2:25, mc.cores=2, ylim=c(-3,4))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "pearson", norm.method="01", out.path="Clustering/PAM_DESeq/pearson/", out.name="DESeq_pearson_01", prior.nr.cl=2:25, mc.cores=1, ylim=c(-0.2,1.2))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "pearson", norm.method="none", out.path="Clustering/PAM_DESeq/pearson/", out.name="DESeq_pearson_none", prior.nr.cl=2:25, mc.cores=1, ylim=NULL)


PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="norm", out.path="Clustering/PAM_DESeq/euclidian/", out.name="DESeq_euclidian_norm", prior.nr.cl=2:25, mc.cores=1, ylim=c(-3,4))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="01", out.path="Clustering/PAM_DESeq/euclidian/", out.name="DESeq_euclidian_01", prior.nr.cl=2:25, mc.cores=1, ylim=c(-0.2,1.2))
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="none", out.path="Clustering/PAM_DESeq/euclidian/", out.name="DESeq_euclidian_none", prior.nr.cl=2:25, mc.cores=1, ylim=NULL)
...
PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="norm", out.path="Clustering/PAM_DESeq_CPM/euclidian/", out.name="DESeq_CPM_EU_norm", prior.nr.cl=2:30, mc.cores=5, ylim=c(-3,6))

PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="01", out.path="Clustering/PAM_DESeq_CPM/euclidian/", out.name="DESeq_CPM_EU_01", prior.nr.cl=2:5, mc.cores=2, ylim=c(-0.2,1.2))


PAM.clustering(pam.x, new.samps, genes.full.description, dist.method= "euclidian", norm.method="01", out.path="Clustering/PAM_DESeq_CPM/euclidian/", out.name="DESeq_spearman_01", prior.nr.cl=2:25, mc.cores=1, ylim=c(-0.2,1.2))
...
kmeans.clustering <- function(pam.x, new.samps, genes.full.description, norm.method="norm", out.path="Clustering/Kmeans/", out.name="", prior.nr.cl=2:5, mc.cores=1, ylim=c(-2,6), colors=c("white","blue")){
  ...
  Gap.inds <- mclapply(1:(length(pam.obj.all)-1), function(p){ 
    ...
    pam.indexGap.pc  <- index.Gap(x=pam.x, clall=clall , method="k-means", centrotypes="centroids", reference.distribution="pc")$diffu
    pam.indexGap.unif <- index.Gap(x=pam.x, clall=clall, method="k-means", centrotypes="centroids", reference.distribution="unif")$diffu
  ...
  save(pam.obj.all, pam.indexG1, pam.indexGap.pc, pam.indexGap.unif, file=paste0(out.path,"/",out.name,"_pam_obj.Rdata"))
...
kmeans.clustering(pam.x, new.samps, genes.full.description, norm.method="01", out.path="Clustering/Kmeans_CPM_more_iterations/", out.name="CPM_01", prior.nr.cl=2:40, mc.cores=1, ylim=c(-0.1,1.1))
...
#library(ggplot2)
library(RColorBrewer)
source("R/heatmap2.r") # function heatmap.2axis


HC.clustering <- function(x.obj, new.samps, norm.method="norm", dist.method="spearman", cl.method="average", out.path="Clustering/HC/",  out.name=""){
  ...
  if(norm.method!="none")
    x.obj <- normalize.counts(counts=x.obj, norm.method=norm.method)
  
  cat("Calculating distance matrix...\n")
  
  if(dist.method %in% c("spearman" ,"kendall") ){
    
    sim.obj.g <- cor(t(x.obj), method=dist.method)
    dist.obj.g <- as.dist(1-sim.obj.g)
    sim.obj.s <- cor(x.obj, method=dist.method)
    dist.obj.s <- as.dist(1-sim.obj.s)
    
  } else {
    dist.obj.g <- dist(x=x.obj, method = dist.method)
    dist.obj.s <- dist(t(x.obj), method = dist.method)
  }
  ...
  png(paste0(out.path,"/", out.name ,"HMg.png"), w=1000, h=1000)
  ...
  png(paste0(out.path,"/", out.name ,"HMgs.png"), w=1000, h=1000)
...
x.obj <- count.data$vsd[selected.genes$var.genes.vs, ]

HC.clustering(x.obj, new.samps, norm.method="norm", dist.method="spearman", cl.method="average", out.path="Clustering/HC_DESeq/spearman/",  out.name="DESeq_spearman_norm_average_")

HC.clustering(x.obj, new.samps, norm.method="none", dist.method="spearman", cl.method="average", out.path="Clustering/HC_DESeq/spearman/",  out.name="DESeq_spearman_none_average_")

HC.clustering(x.obj, new.samps, norm.method="01", dist.method="spearman", cl.method="average", out.path="Clustering/HC_DESeq/spearman/",  out.name="DESeq_spearman_01_average_")


HC.clustering(x.obj, new.samps, norm.method="norm", dist.method="euclidian", cl.method="average", out.path="Clustering/HC_DESeq/euclidian/",  out.name="DESeq_euclidian_norm_average_")

HC.clustering(x.obj, new.samps, norm.method="norm", dist.method="euclidian", cl.method="ward", out.path="Clustering/HC_DESeq/euclidian/",  out.name="DESeq_euclidian_norm_ward_")

HC.clustering(x.obj, new.samps, norm.method="norm", dist.method="euclidian", cl.method="complete", out.path="Clustering/HC_DESeq/euclidian/",  out.name="DESeq_euclidian_norm_complete_")
...
d <- t(count.data$vsd[selected.genes$var.genes.vs, ])[,1:1000]

results = ConsensusClusterPlus(d, maxK=20, reps=10, pItem=0.8, pFeature=1, clusterAlg="pam", distance="spearman", plot="pngBMP")

# cluster and item consensus
icl = calcICL(results, plot="pngBMP")
