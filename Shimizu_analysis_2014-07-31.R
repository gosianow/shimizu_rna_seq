#####################################################################################################
# repeat all analysis on new data from 16 Jun 2014: data2
...
samps <- read.table("_Data/sample_list.csv", sep=";", stringsAsFactors=FALSE, header=TRUE)
head(samps)
names(samps)
...
sm <- read.table("_Data/meteor/Soil_Moisture.csv", sep=";", stringsAsFactors=FALSE, header=TRUE)
head(sm)
sm <- sm[1:271,]
...
### plots of raw data
dir.create(path="Plots_Covariates", showWarnings=FALSE, recursive=TRUE)

lwd=2

pdf("Plots_Covariates/Soil_moisture.pdf", width = 14, height = 7)
plot(sm$time_nr, sm$DE970, type="l", col=trees.order$color[trees.order$tree_ID=="970"], ylim=c(-0.1,1.1), main="Soil Moisture", ylab="Soil moisture", xlab="Time", xaxt = "n", lwd=lwd)
axis(side=1, at=month.days[,2], labels=month.days[,1])
lines(sm$time_nr, sm$C970, col=trees.order$color[trees.order$tree_ID=="970"], lty=2, lwd=lwd)
lines(sm$time_nr, sm$DE8266, col=trees.order$color[trees.order$tree_ID=="8266"], lwd=lwd)
lines(sm$time_nr, sm$C8266, col=trees.order$color[trees.order$tree_ID=="8266"], lty=2, lwd=lwd)
lines(sm$time_nr, sm$DE8212, col=trees.order$color[trees.order$tree_ID=="8212"], lwd=lwd)
lines(sm$time_nr, sm$C8212a, col=trees.order$color[trees.order$tree_ID=="8212"], lty=2, lwd=lwd)
lines(sm$time_nr, sm$C8212b, col=trees.order$color[trees.order$tree_ID=="8212"], lty=3, lwd=lwd)
legend("bottomleft", c("DE970", "C970","DE8266","C8266" ,"DE8212","C8212a" ,"C8212b"), col=c(trees.order$color[trees.order$tree_ID=="970"], trees.order$color[trees.order$tree_ID=="970"], trees.order$color[trees.order$tree_ID=="8266"], trees.order$color[trees.order$tree_ID=="8266"], trees.order$color[trees.order$tree_ID=="8212"], trees.order$color[trees.order$tree_ID=="8212"], trees.order$color[trees.order$tree_ID=="8212"]), lty=c(1,2,1,2,1,2,3), cex=1, lwd=lwd)
dev.off()


pdf("Plots_Covariates/Soil_moisture_Control.pdf", width = 10, height = 5)
...
pdf("Plots_Covariates/Soil_moisture_DE970.pdf", width = 10, height = 5)
plot(sm$time_nr, sm$DE970, type="l", col=trees.order$color[trees.order$tree_ID=="970"], ylim=c(0,1.1), xlim=c(min(all.days.short[,2]), max(all.days.short[,2])), main="DE970", ylab="Soil moisture", xlab="Time",  xaxt = "n", lwd=lwd, cex.axis=1.2, cex.lab=1.4)
axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
abline(v=new.samps$time_nr[new.samps$tree_ID==970], col="grey")
lines(sm$time_nr, sm$C970, col=trees.order$color[trees.order$tree_ID=="970"], lty=2, lwd=lwd)
dev.off()

pdf("Plots_Covariates/Soil_moisture_DE8266.pdf", width = 10, height = 5)
plot(sm$time_nr, sm$DE8266, type="l", col=trees.order$color[trees.order$tree_ID=="8266"], ylim=c(0,1.1), xlim=c(min(all.days.short[,2]), max(all.days.short[,2])), main="DE8266", ylab="Soil moisture", xlab="Time",  xaxt = "n", lwd=lwd, cex.axis=1.2, cex.lab=1.4)
axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
abline(v=new.samps$time_nr[new.samps$tree_ID==8266], col="grey")
lines(sm$time_nr, sm$C8266, col=trees.order$color[trees.order$tree_ID=="8266"], lty=2, lwd=lwd)
dev.off()

pdf("Plots_Covariates/Soil_moisture_DE8212.pdf", width = 10, height = 5)
plot(sm$time_nr, sm$DE8212, type="l", col=trees.order$color[trees.order$tree_ID=="8212"], ylim=c(0,1.1), xlim=c(min(all.days.short[,2]), max(all.days.short[,2])), main="DE8212", ylab="Soil moisture", xlab="Time",  xaxt = "n", lwd=lwd, cex.axis=1.2, cex.lab=1.4)
axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
...
new.samps.control <- interpolate.and.roll(intrp.x=control.l$x, intrp.y=control.l$y, intrp.points=all.days[,2], table1=new.samps[new.samps$drough.control=="control",], roll.days=c(7, 14, 28), col.values="Soil.Moisture", plot.name="Control", color=trees.order$color[trees.order$tree_ID=="1377"])

new.samps.DE970 <- interpolate.and.roll(intrp.x=sm$time_nr, intrp.y=sm$DE970, intrp.points=all.days[,2], table1=new.samps[new.samps$tree_ID == 970,], roll.days=c(7, 14, 28), col.values="Soil.Moisture", plot.name="DE970", color=trees.order$color[trees.order$tree_ID=="970"])

new.samps.DE8212 <- interpolate.and.roll(intrp.x=sm$time_nr, intrp.y=sm$DE8212, intrp.points=all.days[,2], table1=new.samps[new.samps$tree_ID == 8212,], roll.days=c(7, 14, 28), col.values="Soil.Moisture", plot.name="DE8212", color=trees.order$color[trees.order$tree_ID=="8212"])

new.samps.DE8266 <- interpolate.and.roll(intrp.x=sm$time_nr, intrp.y=sm$DE8266, intrp.points=all.days[,2], table1=new.samps[new.samps$tree_ID == 8266,], roll.days=c(7, 14, 28), col.values="Soil.Moisture", plot.name="DE8266",  color=trees.order$color[trees.order$tree_ID=="8266"])
...
wp <- read.table("_Data/meteor/Water_potential_from_Inoue.csv", sep=";", header=T)
head(wp)
names(wp)
...
for(i in 1:length(tree.ids)){
  ...
  pdf(paste("Plots_Covariates/Water_potential_", exp.tree.ids[i], ".pdf", sep=""), width = 10, height = 5)
  plot(wp$time_nr[!is.na(wp[,tree.sampl])], wp[!is.na(wp[,tree.sampl]), tree.sampl], type="l", col=trees.order$color[trees.order$tree_ID==tree.ids[i]], ylim=c(0.2,0.8), xlim=c(min(all.days.short[,2]), max(all.days.short[,2])), main=exp.tree.ids[i], ylab="Mean Water Potential", xlab="Time", xaxt = "n", lwd=4, cex.axis=1.2, cex.lab=1.4)
  axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
...
pdf(paste("Plots_Covariates/Water_potential.pdf", sep=""), width = 10, height = 5)
i=1
tree.sampl <- paste(exp.tree.ids[i], "sMean", sep="")
plot(wp$time_nr[!is.na(wp[,tree.sampl])], wp[!is.na(wp[,tree.sampl]), tree.sampl], type="l", col=trees.order$color[trees.order$tree_ID==tree.ids[i]], ylim=c(0.2,0.8), xlim=c(min(wp$time_nr), max(wp$time_nr)), main="Water Potential", ylab="Mean Water Potential", xlab="Time", xaxt = "n", lwd=4, cex.axis=1.2, cex.lab=1.4)
axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
for(i in 2:length(tree.ids)){
  # i=5
  tree.sampl <- paste(exp.tree.ids[i], "sMean", sep="")
  lines(wp$time_nr[!is.na(wp[,tree.sampl])], wp[!is.na(wp[,tree.sampl]), tree.sampl], col=trees.order$color[trees.order$tree_ID==tree.ids[i]], lwd=4)
}
...
pdf(paste("Plots_Covariates/Water_potential_May.pdf", sep=""), width = 10, height = 5)
i=1
tree.sampl <- paste(exp.tree.ids[i], "sMean", sep="")
plot(wp$time_nr[!is.na(wp[,tree.sampl])], wp[!is.na(wp[,tree.sampl]), tree.sampl], type="l", col=trees.order$color[trees.order$tree_ID==tree.ids[i]], ylim=c(0.2,0.8), xlim=c(min(wp$time_nr), max(wp$time_nr)), main="Water Potential", ylab="Mean Water Potential", xlab="Time", xaxt = "n", lwd=4, cex.axis=1.2, cex.lab=1.4)
axis(side=1, at=month.days[,2], labels=month.days[,1], cex.axis=1.2)
...
for(i in 1:length(tree.ids)){
  # i=1
  tree.sampl <- paste(exp.tree.ids[i], "sMean", sep="")
  new.samps.wp <- rbind(new.samps.wp, new.samps.wp.tmp <- interpolate.and.roll(intrp.x=wp$time_nr[!is.na(wp[,tree.sampl])], intrp.y=wp[!is.na(wp[,tree.sampl]),tree.sampl], intrp.points=all.days[,2], table1=new.samps[new.samps$tree_ID == tree.ids[i],] , roll.days=c(7, 14, 28), col.values="Water.Potential", plot.name=tree.sampl))
...
tempr <- read.table("_Data/meteor/Lambir_temperature_data.csv", head=T, sep=";")
head(tempr)
...
pdf("Plots_Covariates/Temperature_raw.pdf", width = 10, height = 5)
plot(tempr$time_nr, tempr$Temp, pch=".", main="Temperature", xlab="Time", ylab="Temperature")
#abline(v=new.samps$time_nr, col="grey")
dev.off()

pdf("Plots_Covariates/Temperature_over_day.pdf", width = 10, height = 5)
#boxplot(Temp ~ time_hour, data=tempr, xlab="Hour", ylab="Temperature")
beanplot(Temp ~ time_hour, data=tempr, xlab="Hour", ylab="Temperature", what=c(0,1,0,0), col="blue")
dev.off()
...
### lowess for control
dir.create("Plots_Interpolate")
tempr.l <- lowess(tempr.day[,"time_nr"], tempr.day[,"TempAvg"], f=0.1)

pdf("Plots_Interpolate/Temperature_Avg_lowess.pdf", width = 10, height = 5)
plot(tempr.day[,"time_nr"], tempr.day[,"TempAvg"], col="grey", pch=20, main="", ylab="Avg Temperature", xlab="Time",  xaxt = "n")
axis(side=1, at=month.days[,2], labels=month.days[,1])
lines(tempr.l, lwd=4, col="blue")
dev.off()
...
pdf("Plots_Covariates/Temperature_Avg.pdf", width = 10, height = 5)
...
pdf("Plots_Covariates/Temperature_Max.pdf", width = 10, height = 5)
...
pdf("Plots_Covariates/Temperature_Min.pdf", width = 10, height = 5)
...
new.samps <- merge(new.samps, tempr.day, by="time_nr")
...
Athaliana.flowering.genes <- read.table("_Data/genes_descr_control/gene_list_flowering_related_MolEcol_Athaliana_S4.csv", sep=",", header=T, stringsAsFactors=FALSE, skip=1)
...
### Drought regulated genes 
UP.genes <- read.table("_Data/genes_descr_control/mDr_Day10_drought_up_regulated_genes_Harb_etal.csv", sep=";", header=T, stringsAsFactors=FALSE)
DOWN.genes <- read.table("_Data/genes_descr_control/mDr_Day10_drought_down_regulated_genes_Harb_etal.csv", sep=";", header=T, stringsAsFactors=FALSE)
...
x <- read.table("_Data/new_data_16Jun/rsem_expected_count_table.csv", sep=",", header=T, row.names=1)
x <- x[-c(1:2), -c(1, 56)]

# round the expected counts
x <- ceiling(x)

new.samps$sample_name_short <- substr(new.samps$sample_name, 1, nchar(new.samps$sample_name)-9)
x <- x[, new.samps$sample_name_short]
colnames(x) <- new.samps$sample_name
...
############### files with gene matches

#### connection between S. leprosula and D. aromatica (drought experiment assembled)

SD <- read.table("_Data/new_data_16Jun/sle64_s700_scaf_filtered_intronless_on_Trinity.txt", stringsAsFactors=FALSE, sep = ",")
colnames(SD) <- c("SL_id", "DA_id")


#### match A.thaliana with S.leprosula
library(limma)
AT.id <- read.table("_Data/new_data_16Jun/Athaliana_vs_sle699_genes.txt", stringsAsFactors=FALSE)
AT.id[,1] <- strsplit2(AT.id[,1], ".", fixed=T)[, 1]
colnames(AT.id) <- c("SL_id", "AT_id")
head(AT.id)
...
syms <- data.frame(AT_id = names(syms), AT_symbol=syms)


##### get TAIR10_functional_descriptions
tfd <- read.table("_Data/genes_descr_control/TAIR10_functional_descriptions",sep="\t", header=TRUE, quote="", comment.char="", stringsAsFactors=FALSE)
t10id <- strsplit2(tfd$Model_name, ".", fixed=TRUE)[,1]
tfd2 <- data.frame(AT_id = t10id, Description = tfd$Short_description)


#### create gene full description file
genes.description <- merge(AT.id, syms, by="AT_id", all.x=TRUE)
genes.description <- merge(genes.description, Athaliana.flowering.genes, by = "AT_id", all.x=TRUE)
genes.description <- merge(genes.description, Drought.genes, by = "AT_id", all.x=TRUE)
genes.description <- merge(genes.description, tfd2, by = "AT_id", all.x=TRUE)
genes.description <- unique(genes.description)
head(genes.description)



write.table(genes.description, "_Data/new_data_16Jun/genes_description.xls", sep="\t", quote = FALSE, row.names = FALSE, col.names=TRUE)



##### genes description for genes from x

genes.full.description <- data.frame(DA_id=rownames(x))
genes.full.description <- merge(genes.full.description, SD, by="DA_id", all.x=TRUE)
genes.full.description <- merge(genes.full.description, genes.description, by="SL_id", all.x=TRUE)
genes.full.description <- genes.full.description[, c("DA_id","AT_id","SL_id","AT_symbol","AT_name","Flowering","Drought_regulation","Description3","Description")]
write.table(genes.full.description, "_Data/new_data_16Jun/data2_genes_full_description.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)


AT.id <- genes.full.description[!is.na(genes.full.description$AT_id), c("DA_id", "AT_id")]
...
plotRLE(cpm(d, normalized.lib.sizes=TRUE), col = new.samps$tree_col, outline = FALSE, main="CPM & normalized library sizes")
dev.off()
...
write.table(data.frame(DA_id=rownames(d.cpm), d.org$counts), "_Data/new_data_16Jun/data2.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)


write.table(data.frame(DA_id=rownames(d.cpm), d.cpm), "_Data/new_data_16Jun/data2_cpm.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

write.table(data.frame(DA_id=rownames(d.cpm.l), d.cpm.l), "_Data/new_data_16Jun/data2_cpm_log.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
...
pdf(paste0("Plots_of_flowering_genes/" , "Flowering_genes_from_S4_table_fl" ,".pdf"), h=5, w=10)
#info.table <- NULL
  ...
  if(length(genes!=0)){
    for(j in 1:length(genes)){
      ...
      for(t in trees.order$legend){
        ...
        lines(new.samps$time_nr[new.samps$tree_legend == t], d.cpm[genes[j], new.samps$tree_legend == t] , col=trees.order$color[trees.order$legend==t], type="b", pch=trees.order$pch[trees.order$legend==t], cex=trees.order$cex[trees.order$legend==t], lwd=4) 
      }
      legend("topright", legend = trees.order$legend, col=trees.order$color, cex=0.7, text.col=trees.order$color)
...
for(v in plot.vars){
  ...
  for(t in trees.order$legend){
    lines(new.samps$time_nr[new.samps$tree_legend==t], new.samps[new.samps$tree_legend==t, v], col=trees.order$color[trees.order$legend==t], type="b", pch=trees.order$pch[trees.order$legend==t], cex=trees.order$cex[trees.order$legend==t], lwd=3) 
  }
  axis(side=1, at=month.days[,2], labels=month.days[,1])
...
plot.vars <- c("Soil.Moisture")
for(v in plot.vars){
  plot(0, type="n", xlim=c(min(new.samps$time_nr), max(new.samps$time_nr)), ylim=c(min(na.omit(new.samps[,v])), max(na.omit(new.samps[,v]))), xlab="Time", ylab=v, xaxt = "n")
  for(t in trees.order$legend[trees.order$condition=="DE"]){
    lines(new.samps$time_nr[new.samps$tree_legend==t], new.samps[new.samps$tree_legend==t, v], col=trees.order$color[trees.order$legend==t], type="b", pch=trees.order$pch[trees.order$legend==t], cex=trees.order$cex[trees.order$legend==t], lwd=3) 
  }
  axis(side=1, at=month.days[,2], labels=month.days[,1])
  legend("bottomright", legend = trees.order$legend[trees.order$condition=="DE"], col=trees.order$color[trees.order$condition=="DE"], cex=0.7, text.col=trees.order$color[trees.order$condition=="DE"])
...
pdf(paste0("Plots_of_flowering_genes/" , "Flowering_genes_from_S4_table_fl_log" ,".pdf"), h=5, w=10)
...
for(g in 1:nrow(AT.genes)){
  ...
  if(length(genes!=0)){
    for(j in 1:length(genes)){
      ...
      for(t in trees.order$legend){
        # t=trees.order$legend[1]
        lines(new.samps$time_nr[new.samps$tree_legend ==t], d.cpm.l[genes[j], new.samps$tree_legend ==t] , col=trees.order$color[trees.order$legend==t], type="b", pch=trees.order$pch[trees.order$legend==t], cex=trees.order$cex[trees.order$legend==t], lwd=4) 
      }
      axis(side=1, at=month.days[,2], labels=month.days[,1])
      legend("topright", legend = trees.order$legend, col=trees.order$color, cex=0.7, text.col=trees.order$color)
...
neg.contrl.ID <- neg.contrl[!is.na(neg.contrl$Housekeeping), "DA_id"]
...
########### all
kmeans.clustering(pam.x, new.samps, genes.full.description, colors.wp, norm.method="norm", out.path="Clustering/Kmeans_all/", out.name="CPM_norm_all", prior.nr.cl=10:50, mc.cores=15, ylim=c(-2, 5), clustering.samps="all", colors=c("yellow","blue"))
...
kmeans.clustering(pam.x[,drought.samp], new.samps[drought.samp,], genes.full.description, colors.wp, norm.method="norm", out.path="Clustering/Kmeans_drought_trees/", out.name="CPM_norm_drought", prior.nr.cl=5:40, mc.cores=20, ylim=c(-2, 5), clustering.samps="all", colors=c("yellow","blue"))
...
cars.spl <- smooth.spline(speed, dist)
lines(cars.spl, col = "blue")
lines(smooth.spline(speed, dist, spar=0.5), col = "red")
lines(smooth.spline(speed, dist, spar=0.1), col = "green")
