...
run.edgeR.robust <- function(x, new.samps, model.formula, varialbs, elim.samps, FDR=0.1, plots.path="Plots_RUN_edgeR_robust", plot.name="", varialbs.plot, fit.clr = 1, LRTcoef=2, trees.order=trees.order, genes.full.description=genes.full.description, AT.id=AT.id){ 
...
#   new.samps.tmp$FUL_gene <- log(d.cpm["GID056430_3537430",]+eps)
  ...
  # estimate dispersion  
  d <- estimateGLMRobustDisp(d, design, prior.df = 10)  
  # glmFit
  fit <- glmFit(d,design)
  # glmLRT
