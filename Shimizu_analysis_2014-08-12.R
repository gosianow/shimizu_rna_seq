#####################################################################################################
# repeat all analysis on new data from 16 Jun 2014 
# and updated gene match from 5 Aug 2014 - best_hit_blast_result_Sl_protein_to_Athaliana.txt
...
############### files with control genes and gene desriptions
...
library(limma)
AT.id <- read.table("_Data/new_data_16Jun/best_hit_blast_result_Sl_protein_to_Athaliana.txt", stringsAsFactors=FALSE)
AT.id <- AT.id[,c(1,2)]
AT.id[,2] <- strsplit2(AT.id[,2], ".", fixed=T)[, 1]
...
############## counts on S.leprosula genome

x <- read.table("_Data/new_data_16Jun/kevin_genome_star_htseq_count.csv", sep=",", header=T, row.names=1)
x <- x[-c(1:3), -c(1)]
x <- x[, new.samps$sample_name]

rownames(x) <- strsplit2(rownames(x), "(", fixed=TRUE)[,1]
...
##### genes description for all SL genes
genes.full.description <- data.frame(SL_id=rownames(x))
genes.full.description <- merge(genes.full.description, genes.description, by="SL_id", all.x=TRUE)

write.table(genes.full.description, "_Data/new_data_16Jun/genes_full_description.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
...
write.table(data.frame(SI_id=rownames(d.cpm), d.cpm), "_Data/new_data_16Jun/data1_cpm.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

write.table(data.frame(SI_id=rownames(d.cpm.l), d.cpm.l), "_Data/new_data_16Jun/data1_cpm_log.xls", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
