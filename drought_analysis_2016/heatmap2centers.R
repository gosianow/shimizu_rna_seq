heatmap.2centers <- function(x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE,
    ...
    lmat = NULL, lhei = NULL, lwid = NULL, extrafun = NULL, col.axis.x = "black", ...)
{
    ...
    if (is.null(srtCol))
			##### ADD colored lables 
		for(i in unique(col.axis.x)){
			
			ii <- which(col.axis.x == i)
			
			iii <- 1:nc
      axis(1, iii[ii], labels = labCol[ii], las = 2, line = -0.5 +
          offsetCol, tick = 0, cex.axis = cexCol, hadj = adjCol[1],
          padj = adjCol[2], col.axis = i)
